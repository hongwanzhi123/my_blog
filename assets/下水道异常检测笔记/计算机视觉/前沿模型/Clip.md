...
如果你想让它识别牙齿病灶、工业缺陷、车辆、螺丝、裂纹，就需要重新收集数据、标注类别、训练模型。
...
也就是说，CLIP 不是只会识别训练时固定的几个类别，而是可以根据你提供的文字描述进行判断。
...
你现在的 AlphaDent 牙齿病理分割项目，核心模型还是：
...
CLIP 最典型的能力是 zero-shot 图像分类。比如我不需要重新训练分类头，只需要把类别写成 prompt，例如 a photo of a cat、a photo of a dog，然后计算图片特征和这些文本特征的相似度，选择相似度最高的类别作为预测结果。
