...
例如模型把猫和旁边的狗一起分出来了，你可以在狗身上点一个负点，告诉模型：
...
模型：我只认识训练过的类别，比如 road、car、person
输入图像后，我给每个像素分配类别

SAM 像这样工作：

用户：我要这个区域
SAM：根据你的点/框/mask，把这个区域切出来
...
如果你有大量专业标注数据，要在固定场景做到极致精度，U-Net 可能更合适。

如果你想快速获得 mask、辅助标注、处理开放场景，SAM 更有优势。
...
我训练好识别 person、car、dog，然后自动检测并分割它们
...
SAM3：
    你告诉我概念，比如 yellow school bus
    我找到所有匹配对象，并分割、跟踪它们

Meta 官方博客也将 SAM 3 描述为用于图像和视频中对象检测、分割和跟踪的统一模型，支持 text、exemplar 和 visual prompts。
...
例如你想训练一个轻量化 U-Net 部署到移动端，可以用 SAM 辅助生成训练数据。

## 二十七、SAM 是否适合直接训练你的业务模型？
